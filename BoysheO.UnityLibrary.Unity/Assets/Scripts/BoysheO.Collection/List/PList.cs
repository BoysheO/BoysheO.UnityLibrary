...
namespace BoysheO.Collection
{
    ...
    public partial class PList<T> : IList<T>, IList, IReadOnlyList<T>
    {
        ...
        //
        public PList(int capacity, ArrayPool<T>? pool = null)
        {
            if (capacity < 0)
        ...
        public int Capacity
        {
            ...
            set
            {
                ...
                if (value != _items.Length)
                {
                    if (value > 0)
                    {
                        RefArrayPoolUtil.Resize(ref _items, _size, value, _pool);
        ...
        public PList<TOutput> ConvertAll<TOutput>(Converter<T, TOutput> converter)
        {
            if (converter == null)
            {
            ...
            PList<TOutput> list = new PList<TOutput>(_size);
        ...
        internal void Grow(int capacity)
        {
            ...
            //Unity的这个net版本没这个MaxLength。交给后面resize时报错吧...
        ...
        {
            TryFind(match, out var t);
            return t;
        }

        public PList<T> FindAll(Func<T, bool> match)
        {
            if (match == null)
            {
        ...
        {
            TryFindLast(match, out var value);
            return value;
        }
        ...
        IEnumerator<T> IEnumerable<T>.GetEnumerator() =>
            Count == 0 ? CollectionHelper<T>.EmptyEnumerator : GetEnumerator();

        IEnumerator IEnumerable.GetEnumerator() => ((IEnumerable<T>) this).GetEnumerator();

        public PList<T> GetRange(int index, int count)
        {
            if (index < 0)
            {
                ThrowHelper.ThrowIndexArgumentOutOfRange_NeedNonNegNumException();
            ...
            PList<T> list = new PList<T>(count);
        ...
        /// <summary>
        /// Creates a shallow copy of a range of elements in the source <see cref="PList{T}" />.
        /// </summary>
        /// <param name="start">The zero-based <see cref="PList{T}" /> index at which the range starts.</param>
        /// <param name="length">The length of the range.</param>
        /// <returns>A shallow copy of a range of elements in the source <see cref="PList{T}" />.</returns>
        /// <exception cref="ArgumentOutOfRangeException">
        /// <paramref name="start" /> is less than 0.
        /// -or-
        /// <paramref name="length" /> is less than 0.
        /// </exception>
        /// <exception cref="ArgumentException"><paramref name="start" /> and <paramref name="length" /> do not denote a valid range of elements in the <see cref="PList{T}" />.</exception>
        public PList<T> Slice(int start, int length) => GetRange(start, length);
        ...
        public void Sort(Comparison<T> comparison)
        {
            ...
            if (_size > 1)
            {
                ArraySortHelper.Sort(new Span<T>(_items, 0, _size), comparison);
            }
        ...
        //
        /// <summary>
        /// 特别说明：最好不要手工调用，否则可能产生不必要的Copy和GC。
        /// </summary>
        ...
        public struct Enumerator : IEnumerator<T>, IEnumerator
        {
            ...
            private readonly PList<T> _list;
            private int _index;
            private readonly int _version;
            private T? _current;

            internal Enumerator(PList<T> list)
            {
                _list = list;
                _index = 0;
