...
{
    public class PHashSet<T> : ISet<T>, IDisposable
    {
        private readonly PDictionary<T, ValueTuple> _data;

        /// <summary>
        /// 请即用即弃，对集合的操作可能会替换它的引用。（即使源码中没有这样做，也不保证以后源码不重构）
        /// </summary>
        public PDictionary<T, ValueTuple> InternalData => _data;

        public PHashSet() : this(null)
        {
        }

        public PHashSet(IEqualityComparer<T>? equalityComparer = null)
        {
            _data = new PDictionary<T, ValueTuple>(equalityComparer);
        }

        public PDictionary<T, ValueTuple>.KeyCollection.Enumerator GetEnumerator()
        {
        ...
        {
            using var intersected = VHashSet<T>.Rent(_data.Comparer);
        ...
        {
            using var buffer = VHashSet<T>.Rent(_data.Comparer);
        ...
        public void Reset(IEqualityComparer<T>? equalityComparer)
        {
